...
Simple flow: Select Image Folder -> Select Output Excel -> Process All Images
...
from src.config.template_config import EXCEL_COLUMNS
...
class FormToExcelApp(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        ctk.CTkButton(
            row1, text="1. Select Image Folder", width=200,
        ...
        ctk.CTkButton(
            row2, text="2. Select Output Excel", width=200,
        ...
        self.process_btn = ctk.CTkButton(
            action_frame, text="3. Process All Images", height=45,
        ...
        if folder:
            ...
            try:
                ...
                self.folder_label.configure(
                    text=f"{folder}  ({len(files)} images)",
                    text_color="white"
                )
    ...
    def _process_worker(self):
        try:
            ...
            for i, file_path in enumerate(files):
                ...
                self.after(0, lambda fn=filename, idx=i: self._log(
                    f"[{idx+1}/{total}] Processing: {fn}"
                ))
    ...
    def _processing_done(self):
        self.processing = False
        self.process_btn.configure(state="normal", text="3. Process All Images")
